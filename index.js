...
if (cluster.isPrimary || cluster.isMaster) {
    ...
    if (!state.creds.registered && cluster.worker.id === 1) {
    ...
    global.reload = async function(restatConn) {
      ...
      global.conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            console.log(chalk.cyan('┃ ') + chalk.greenBright.bold(`STATUS: ONLINE [WORKER ${cluster.worker.id}]`));
            await cleanSessions();
