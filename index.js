...
if (cluster.isPrimary || cluster.isMaster) {
    ...
    cluster.on('exit', (worker) => {
        console.error(chalk.red(`┃ Hilo ${worker.id} murió. Reiniciando...`));
        cluster.fork();
    });
    ...
    const startBot = async () => {
        if (dbUrlDecoded && !process.argv.includes('--local')) {
            await mongoose.connect(dbUrlDecoded, {
                ...
                const res1 = await global.db.collection('users').deleteMany({
                    $and: [{ id: { $not: /@s\.whatsapp\.net$/ } }, { lid: { $not: /@lid$/ } }]
                });
            ...
            global.Stats = mongoose.model('Stats', new mongoose.Schema({ command: { type: String, unique: true }, globalUsage: { type: Number, default: 0 }, groups: { type: Map, of: Number, default: {} } }, { strict: false }));
        } else {
            activateLocalDB();
        }
        ...
        global.reload = async function(restatConn) {
          ...
          global.conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                console.log(chalk.cyan('┃ ') + chalk.greenBright.bold(`STATUS: ONLINE [Hilo ${cluster.worker.id}]`));
                await cleanSessions();
                ...
                if (mongoose.connection.db) {
                    ...
                    reportsCollection.watch().on('change', async (change) => {
                        if (change.operationType === 'insert') {
                            ...
                            const opt = { mentions: [data.sender], contextInfo: { mentionedJid: [data.sender], externalAdReply: { title: `CENTRAL DE REPORTES`, body: `Remitente: ${data.pushName}`, mediaType: 1, thumbnailUrl: 'https://dix.lat/logo.png', sourceUrl: 'https://dix.lat' } } };
                            if (data.media) {
                                const buffer = Buffer.from(data.media, 'base64');
                                await conn.sendMessage('120363424997886266@g.us', { [/image/.test(data.mime) ? 'image' : 'video']: buffer, caption: reportMsg, ...opt });
                            } else {
                                await conn.sendMessage('120363424997886266@g.us', { text: reportMsg, ...opt });
                            }
                ...
                const updateStatus = async () => {
                    try {
                        ...
                        await conn.query({ tag: 'iq', attrs: { to: '@s.whatsapp.net', type: 'set', xmlns: 'status' }, content: [{ tag: 'status', attrs: {}, content: Buffer.from(`KIRITO BOT MD | ${time}`, 'utf-8') }] });
                    } catch {}
                };
                updateStatus();
