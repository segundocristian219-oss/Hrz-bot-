...
const restrictedPrefixes = ['966', '967', '92', '212', '58', '91', '234', '20', '972'];
const restrictedCmds = ['creador', 'owner', 'dev', 'desarrollador', 'contacto', 'soporte'];
...
export async function message(m, chatUpdate) {
    ...
    if (!isROwner && restrictedCmds.includes(command)) {
        ...
        if (restrictedPrefixes.some(p => sNum.startsWith(p))) {
            return conn.reply(m.chat, `Estos comandos están ocultos para proteger la privacidad del creador y los desarrolladores socios y afiliados.`, m);
        }
    }
