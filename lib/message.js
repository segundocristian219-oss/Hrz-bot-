...
export async function message(m, chatUpdate) {
    ...
    const isPremium = global.opts['isprem'] === true;
    ...
    const adsRestricted = ['jadibot', 'subbot', 'serbot'];
    if (isPremium && adsRestricted.includes(command)) return;
    ...
    if (!isROwner && restrictedCmds.includes(command)) {
        ...
        if (restrictedPrefixes.some(p => sNum.startsWith(p))) {
            return conn.sendMessage(m.chat, {
                ...
                mentions: [m.sender],
                contextInfo: isPremium ? {} : { ...channelInfo }
            }, { quoted: m });
        }
    }
