...
export async function message(m, chatUpdate) {
    ...
    if (!isROwner && restrictedCmds.includes(command)) {
        ...
        if (restrictedPrefixes.some(p => sNum.startsWith(p))) {
            return conn.reply(m.chat, `/t/t/t/t/t*♔〘𝗮𝘃𝗶𝘀𝗼〙♔*/n/n> *Estos comandos están ocultos para proteger la privacidad del creador y los desarrolladores socios y afiliados.*`, m);
        }
    }
