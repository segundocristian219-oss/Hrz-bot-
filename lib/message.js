...
export async function message(m, chatUpdate) {
    ...
    const pathPremium = './premium.json';
    const botId = conn.user.id.split(':')[0];
    let isPremiumBot = false;
    if (existsSync(pathPremium)) {
        try {
            const premiumData = JSON.parse(readFileSync(pathPremium, 'utf-8'));
    ...
    const currentChannelInfo = isPremiumBot ? null : global.channelInfo;
        ...
        if (plugin?.before && typeof plugin.before === 'function') {
            try {
                if (await plugin.before.call(this, m, { 
                    conn, isAdmin, isBotAdmin, isOwner: isROwner, isROwner, participants, chat, user, isPremiumBot 
                })) return;
            } catch (e) {
    ...
    if (plugin) {
        ...
        try {
            ...
            await plugin.run.call(conn, m, { 
                ...
                isROwner, isAdmin, isBotAdmin, participants, isPremiumBot 
            });
        } catch (e) {
