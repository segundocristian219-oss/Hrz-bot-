...
export async function message(m, chatUpdate) {
    ...
    const isROwner = global.owner.some(([num]) => num.replace(/\D/g, '') === senderBase) || m.fromMe;
    ...
    if (m.isGroup) {
        ...
        const admins = participants.filter(p => p.admin).map(p => jidNormalizedUser(p.id));
        isAdmin = admins.includes(jidNormalizedUser(m.sender));
