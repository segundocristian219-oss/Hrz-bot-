    ...
    try {
        if (m?.isGroup) {
            ...
            user = await global.User.findOneAndUpdate(query, updateData, { 
                ...
                if (realSenderId.endsWith('@s.whatsapp.net')) {
                    newUser.id = realSenderId;
                }

                if (rawLid || realSenderId.endsWith('@lid')) {
                    newUser.lid = rawLid || realSenderId;
                }

                if (!newUser.id && !newUser.lid) return;

                user = await global.User.create(newUser);
            } else {
                let update = {};
