...
export async function message(m, chatUpdate) {
    ...
    const msgText = m.text || m.msg?.text || m.msg?.caption || m.msg || '';
    const prefixes = ['#', '.', '/', '!'];
    const usedPrefix = prefixes.find(p => msgText && typeof msgText === 'string' && msgText.startsWith(p));
    
    if (!usedPrefix) return;

    const textAfterPrefix = msgText.slice(usedPrefix.length).trim();
    ...
    if (!global.plugins || !(global.plugins instanceof Map)) return;
    ...
    if (plugin) {
        ...
        try {
            ...
            uploadError(e).then(supportUrl => {
                ...
                conn.sendMessage(m.chat, { text: `*───「 ⚠️ FALLO 」───*\n\n*ID:* #${errorId}\n\n*LOG:*\n\`\`\`${e.message}\`\`\`` }, { quoted: m }).catch(() => null);
            }).catch(() => null);
        }
    }
}
