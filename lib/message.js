...
export async function message(m, chatUpdate) {
    ...
    if (m.isGroup) {
        ...
        if (usedPrefixTemp && !isAdmin && !isROwner) {
            const noPrefixTemp = msgTextTemp.slice(usedPrefixTemp.length).trim();
            const commandTemp = noPrefixTemp.split(/\s+/)[0].toLowerCase();
            const pluginNameTemp = global.plugins instanceof Map 
                ? (global.plugins.has(commandTemp) ? commandTemp : global.aliases.get(commandTemp))
                : (global.plugins[commandTemp] ? commandTemp : Object.keys(global.plugins).find(k => global.plugins[k].alias && global.plugins[k].alias.includes(commandTemp)));

            const pluginTemp = global.plugins instanceof Map ? global.plugins.get(pluginNameTemp) : global.plugins[pluginNameTemp];

            if (pluginTemp?.admin) {
                await cacheManager.get(conn, chatJid, true);
                isAdmin = cacheManager.getAdminStatus(chatJid, m.sender, m.author);
                isBotAdmin = cacheManager.getAdminStatus(chatJid, conn.user.id, conn.user.lid);
                participants = (cacheManager.cache.get(chatJid))?.participants || [];
            }
        }
    }
    ...
    if (m.messageStubType) {
        const metadata = m.isGroup ? await cacheManager.get(conn, m.chat) : {};
    ...
    if (plugin) {
        ...
        try {
            ...
            await conn.sendPresenceUpdate(randomAction, m.chat).catch(() => null);
