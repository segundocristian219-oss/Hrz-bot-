...
export async function message(m, chatUpdate) {
    ...
    const isAntisubCommand = msgText.startsWith('.') && msgText.includes('antisub');

    if (m.isGroup && chat && !isAntisubCommand) {
        if (chat.antisub === true && !isMainBot) return;
        if (chat.antisub === false && isMainBot) return;
    }
