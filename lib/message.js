...
export async function message(m, chatUpdate) {
    ...
    const senderJid = (await getRealJid(conn, m.sender, m)) || m.sender; 
    ...
    // Detección de Dueño mejorada (compara números limpios)
    const isROwner = global.owner.some(([num]) => {
        if (!num) return false;
        const ownerNumber = num.toString().replace(/\D/g, '');
        const senderNumber = m.sender.split('@')[0].split(':')[0];
        const realSenderNumber = senderJid ? senderJid.split('@')[0].split(':')[0] : senderNumber;
        return ownerNumber === senderNumber || ownerNumber === realSenderNumber;
    }) || m.fromMe;
    ...
    if (plugin) {
        ...
        const checkPermissions = (perm) => {
            ...
            // Si el permiso requerido es rowner u owner, devolvemos el valor real
            if (perm === 'rowner' || perm === 'owner') return isROwner;
            // Para otros permisos, si es dueño, le damos paso libre
            return isROwner ? true : perms[perm];
        };
