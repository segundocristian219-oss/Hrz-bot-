...
export async function message(m, chatUpdate) {
    ...
    if (m.isGroup && participants.length > 0) {
        const currentUser = jidNormalizedUser(m.sender);
        const botUser = jidNormalizedUser(conn.user.id);
        const p = participants.find(p => jidNormalizedUser(p.id) === currentUser);
        const b = participants.find(p => jidNormalizedUser(p.id) === botUser);
        isAdmin = !!(p?.admin || p?.isCommunityAdmin || isOwner);
        isBotAdmin = !!(b?.admin || b?.isCommunityAdmin);
    }
