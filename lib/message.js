...
export async function message(m, chatUpdate) {
    ...
    if (m.isGroup && chat && chat.muto && !isROwner) {
        if (isBotAdmin) {
...
global.dfail = (type, m, conn) => {
    const messages = {
        rowner: `> â•°â’ Solo mi creador puede usar este comando.`,
        owner: `> â•°â’ Solo mi creador puede usar este comando.`,
        group: `> â•°âœŽ Este comando sÃ³lo se puede usar en grupos.`,
        private: `De esto solo hablo en privado.`,
        admin: `> â•°â™› SÃ³lo los administradores pueden ejecutar este comando.`,
        botAdmin: `> â•°âœ° Necesito ser administrador.`,
    };
