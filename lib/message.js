...
export async function message(m, chatUpdate) {
    ...
    if (!command) return;
    const pluginName = global.plugins.has(command) ? command : [...global.plugins.values()].find(p => p.alias && (Array.isArray(p.alias) ? p.alias.includes(command) : p.alias === command))?.name || global.aliases.get(command);
