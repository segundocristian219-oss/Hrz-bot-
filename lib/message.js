...
const senderJid = (await getRealJid(conn, m.sender, m)) || m.sender; 
    ...
    // Comparamos contra el sender original y el real JID
    const senderNumber = m.sender.split('@')[0].split(':')[0];
    const realNumber = senderJid ? senderJid.split('@')[0].split(':')[0] : senderNumber;
    
    return cleanNum === senderNumber || cleanNum === realNumber;
}) || m.fromMe;
