...
export async function message(m, chatUpdate) {
    ...
    if (!chat) chat = await global.Chat.create({ id: chatJid });

    let user = await global.User.findOne({ id: m.sender });
    if (!user) user = await global.User.create({ id: m.sender, name: m.pushName || "" });
    ...
    // --- DETECCIÓN DE ADMINS (LÓGICA RECUPERADA) ---
    let participants = [];
    let isAdmin = false;
