...
export async function message(m, chatUpdate) {
    ...
    if (m.isGroup) {
        ...
        if (!chat) {
            chat = await global.Chat.create({ 
                id: chatJid, isBanned: false, welcome: true, detect: true, antiLink: true,
    ...
    if (m.isGroup && chat?.mutos?.includes(m.sender)) {
        const groupMetadata = await getGroupMetadata(conn, chatJid);
        const participants = groupMetadata.participants || [];
        
        const checkAdmin = participants.find(p => jidNormalizedUser(p.id) === jidNormalizedUser(m.sender));
        const isOwnerMuted = global.owner.some(([num]) => num.replace(/\D/g, '') === senderJid.split('@')[0]);

        if (!checkAdmin?.admin && !isOwnerMuted && !m.fromMe) {
    ...
    for (const plugin of allPlugins) {
        if (plugin?.before && typeof plugin.before === 'function') {
            ...
            } catch (e) { console.error(chalk.red(`Error en before:`), e); }
        }
    }
