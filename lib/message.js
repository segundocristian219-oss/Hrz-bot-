...
export async function message(m, chatUpdate) {
        ...
        if (m.sender.endsWith('@s.whatsapp.net') || m.sender.endsWith('@lid')) {
            ...
            if (!user) {
                user = await global.User.create({ 
                    ...
                    exp: 0, warnAntiLink: 0, col: 10, lastSeen: new Date()
                });
            }
        }
    } catch (e) {
    ...
    const text = m.text.slice(m.text.indexOf(args[0]) + args[0].length).trim();
    ...
    if (plugin) {
        ...
        try {
            if (global.Stats) {
                const cmdName = pluginName || command;
                const groupKey = m.chat.replace(/\./g, '_');
                global.Stats.updateOne(
                    { command: cmdName },
                    { 
