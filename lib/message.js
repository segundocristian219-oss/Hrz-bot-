...
export async function message(m, chatUpdate) {
    ...
    if (m.isGroup && chat?.mutos?.includes(m.sender) && !isAdmin && !isROwner) {
        if (isBotAdmin) {
    ...
    const noPrefix = usedPrefix ? msgText.slice(usedPrefix.length).trim() : msgText;
    ...
    if (plugin) {
        if (!usedPrefix && !plugin.customPrefix && !plugin.fixedPrefix) {
            const hasPrefixMatch = prefixes.some(p => msgText.startsWith(p));
            if (hasPrefixMatch) return;
        }
