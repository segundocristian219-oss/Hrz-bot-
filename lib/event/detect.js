...
export async function events(conn, m, participants) {
    ...
    if (isWelcome && chat.welcome) {
        ...
        const { subject, participants: groupParticipants, owner, creation } = groupMetadata

        const groupName = subject || 'Sistema'
        const memberCount = groupParticipants?.length || '0'
        const ownerNumber = (owner || m.chat.split('-')[0] + '@s.whatsapp.net').split('@')[0]
        const dateCreated = creation ? new Date(creation * 1000).toLocaleDateString('es-HN') : 'Desconocida'
        ...
        let welcomeThumb
        try { 
            welcomeThumb = await conn.profilePictureUrl(who, 'image') 
        } catch { 
            welcomeThumb = thumb 
        }

        return await conn.sendMessage(m.chat, { 
            image: { url: welcomeThumb }, 
            caption: txt, 
            mentions: [who, (owner || m.chat.split('-')[0] + '@s.whatsapp.net')].filter(Boolean) 
        }, { quoted: m })
    }
