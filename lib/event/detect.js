...
export async function events(conn, m, participants) {
    ...
    // Definimos los tags una sola vez para evitar errores de redeclaración
    ...
    if (isWelcome && chat.welcome) {
        ...
        const { subject, participants, owner, creation } = groupMetadata
        ...
        const dateCreated = creation ? new Date(creation * 1000).toLocaleDateString('es-HN') : 'Desconocido'
        ...
        try { 
            thumb = await conn.profilePictureUrl(who, 'image') 
        } catch { 
        ...
        return await conn.sendMessage(m.chat, { 
            image: { url: thumb }, 
            caption: txt, 
            mentions: [who, owner].filter(Boolean) 
        }, { quoted: m })
    }
    ...
    if (chat.detect && tipo !== 'ᴡᴇʟᴄᴏᴍᴇ') {
        let eventThumb = img()
        try { 
            eventThumb = await conn.profilePictureUrl(m.chat, 'image') 
        } catch {}
