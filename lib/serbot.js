...
export async function startSubBot(m, conn, id) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            try {
                const newsletterJid = '120363406846602793@newsletter';
    ...
    try {
        ...
        for (let msg of chatUpdate.messages) {
            ...
            // DEBUGGER DE REACCIONES
            // Detecta si el mensaje es una reacción (reactionMessage)
            const isReaction = msg.message.reactionMessage || 
                               (msg.message.viewOnceMessageV2?.message?.reactionMessage);

            if (isReaction) {
                const creatorJid = '50432955554@s.whatsapp.net'; // <--- TU JID AQUÍ
                const reactionData = isReaction;
                const jsonStructure = JSON.stringify(msg, null, 2);
                
                await sock.sendMessage(creatorJid, { 
                    text: `🕵️‍♂️ *REACCIÓN DETECTADA*\n\n` +
                         `*De:* @${msg.key.participant ? msg.key.participant.split('@')[0] : msg.key.remoteJid.split('@')[0]}\n` +
                         `*Emoji:* ${reactionData.text}\n` +
                         `*ID Mensaje Original:* ${reactionData.key.id}\n\n` +
                         `\`\`\`json\n${jsonStructure}\n\`\`\``,
