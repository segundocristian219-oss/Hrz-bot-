...
export async function startSubBot(m, conn, id) {
        ...
        if (connection === 'open') {
            ...
            try {
                ...
                for (const gid in groups) {
                    if (global.groupCache instanceof Map) global.groupCache.set(gid, groups[gid]);
                }
            } catch (e) {}
    ...
    sock.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            const { events } = await import('./event/detect.js');
            ...
            await Promise.all(chatUpdate.messages.map(async (msg) => {
                ...
                if (msg.messageStubType) {
                    const metadata = (jid && jid.endsWith('@g.us')) ? await getGroupMetadata(sock, jid) : {};
                    await events(sock, msg, metadata.participants || []).catch(() => null);
                    return;
                }
