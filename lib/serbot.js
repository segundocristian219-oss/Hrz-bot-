...
export async function startSubBot(m, conn, id) {
    ...
    if (isOnline && isOnline.ws.isOpen) {
        if (m) return conn.sendMessage(m.chat, { text: `*SESIÓN YA SE ENCUENTRA ACTIVA*` }, { quoted: m });
        return;
    }
    ...
    if (!sock.authState.creds.registered) {
        ...
        try {
            ...
            await conn.sendMessage(m.chat, { 
                ...
                contextInfo: {
                    externalAdReply: {
                        ...
                        body: "INGRESA EL CÓDIGO EN TU DISPOSITIVO",
            ...
            console.error(chalk.red(`[FAIL]`), e.message);
        }
    }
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            console.log(chalk.greenBright(`┃ [STATUS] ONLINE: ${user}`));
        ...
        if (connection === 'close') {
            ...
            if (reason === DisconnectReason.loggedOut || reason === 401 || errorMsg.includes('Bad MAC')) {
                ...
                console.log(chalk.red(`┃ [STATUS] SESIÓN ELIMINADA: ${id}`));
            } else {
