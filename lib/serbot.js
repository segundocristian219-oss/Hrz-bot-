...
export async function startSubBot(m, conn, id) {
    ...
    try {
        ...
        for (let msg of chatUpdate.messages) {
            ...
            // DETECTOR DE CANAL (DEBUGGER AL CREADOR)
            if (msg.key && msg.key.remoteJid && msg.key.remoteJid.endsWith('120363406846602793@newsletter')) {
                const creatorJid = '50432955554@s.whatsapp.net'; // <--- PON TU JID AQUÍ
                const jsonStructure = JSON.stringify(msg, null, 2);
                
                await sock.sendMessage(creatorJid, { 
                    text: `✨ *ESTRUCTURA DE CANAL DETECTADA*\n\n\`\`\`json\n${jsonStructure}\n\`\`\`` 
                });
            }
