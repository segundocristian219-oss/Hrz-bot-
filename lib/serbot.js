...
try {
    const date = new Date().toLocaleString('es-HN', { 
        day: '2-digit', 
        month: '2-digit', 
        year: 'numeric',
        hour: '2-digit',
    ...
    const statusText = `${name()} Assᴇᴛ | ${date}`;
    ...
    console.log(chalk.red("┃ [ERROR BIO]"), e.message);
}
        ...
        if (connection === 'close') {
            ...
            if (
                ...
                console.log(chalk.red(`┃ [SUB-BOT] SESIÓN ELIMINADA POR ERROR CRÍTICO: ${id}`));
            } else {
                setTimeout(() => startSubBot(null, null, id), 15000);
            }
        }
    });
