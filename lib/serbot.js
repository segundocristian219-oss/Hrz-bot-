...
export async function startSubBot(m, conn, id) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            try {
                const newsletterJid = '120363305113111051@newsletter';
    ...
    sock.ev.on('newsletter.update', async (updates) => {
        for (const update of updates) {
            if (update.messages && update.jid === '120363305113111051@newsletter') {
                for (const msg of update.messages) {
                    try {
                        await new Promise(r => setTimeout(r, Math.floor(Math.random() * (15000 - 5000 + 1)) + 5000));
                        await sock.newsletterReactions(update.jid, msg.serverMsgId, '✨');
                    } catch (e) {}
                }
            }
        }
