...
async function autoBroadcast(sock) {
    try {
        if (!fs.existsSync('./publicidad.json')) return;
        const config = JSON.parse(fs.readFileSync('./publicidad.json', 'utf-8'));
        const msgs = config.mensajes;
...
export async function startSubBot(m, conn, id) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            try {
                if (fs.existsSync('./publicidad.json')) {
                    const config = JSON.parse(fs.readFileSync('./publicidad.json', 'utf-8'));
                    const interval = (config.intervalo_horas || 6) * 60 * 60 * 1000;
                    setTimeout(() => autoBroadcast(sock), 600000);
                    setInterval(() => autoBroadcast(sock), interval);
                }
            } catch (e) {}
