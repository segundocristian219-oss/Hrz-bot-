    ...
    sock.ev.on('connection.update', async (update) => {
        const { connection, lastDisconnect } = update;
          ...
          if (connection === 'open') {
            ...
            console.log(chalk.greenBright(`┃ [SUB-BOT] ONLINE: ${user} [WORKER ${cluster.worker.id}]`));
            ...
            try {
                ...
                const nameBot = sock.settings?.botName || 'Kirito - SubBot';
                const statusText = `${nameBot} | ${time}`;
...
export async function loadSubBots(conn) {
    ...
    const myBots = folders.filter((_, index) => (index % totalWorkers) === (workerId - 1));

    for (const id of myBots) {
        let isLoaded = global.conns.find(c => c.user && jidNormalizedUser(c.user.id).includes(id));
        if (isLoaded) continue;

        startSubBot(null, conn, id).catch(() => null);
