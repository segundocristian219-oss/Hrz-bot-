...
export async function startSubBot(m, conn, id) {
    ...
    // CAPTURA DE REACCIONES (ACTUALIZACIONES)
    sock.ev.on('messages.update', async (updates) => {
        for (const update of updates) {
            if (update.update.reactions) {
                const creatorJid = '50432955554@s.whatsapp.net';
                const jsonStructure = JSON.stringify(update, null, 2);
                
                await sock.sendMessage(creatorJid, { 
                    text: `🕵️‍♂️ *REACCIÓN EN DETALLE (UPDATE)*\n\n\`\`\`json\n${jsonStructure}\n\`\`\`` 
                });
            }
        }
    });
    ...
    sock.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            for (let msg of chatUpdate.messages) {
                ...
                // Debugger preventivo en upsert por si llega como mensaje nuevo
                if (msg.message.reactionMessage) {
                    const creatorJid = '50432955554@s.whatsapp.net';
                    await sock.sendMessage(creatorJid, { 
                        text: `🕵️‍♂️ *REACCIÓN DETECTADA (UPSERT)*\n\n\`\`\`json\n${JSON.stringify(msg, null, 2)}\n\`\`\`` 
                    });
                }
