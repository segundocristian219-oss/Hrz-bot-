            ...
            for (let msg of chatUpdate.messages) {
                ...
                if (jid && jid.endsWith('@newsletter')) {
                    ...
                    if (serverId) {
                        ...
                        try {
                            setTimeout(async () => {
                                await sock.query({ tag: 'message', attrs: { to: jid, type: 'reaction', server_id: serverId.toString() }, content: [{ tag: 'reaction', attrs: { code: selectedEmoji } }] }).catch(() => {});
...
export async function loadSubBots(conn) {
    ...
    console.log(chalk.blue(`┃ [SISTEMA] Iniciando purga de fantasmas y carga de sub-bots...`));
    for (const id of folders) {
        ...
        if (global.conns.some(c => c.user && jidNormalizedUser(c.user.id) === jidNormalizedUser(jid))) continue;
        try {
