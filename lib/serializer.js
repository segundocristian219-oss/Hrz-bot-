...
export const smsg = async (conn, m) => {
    ...
    if (!conn.downloadM) {
        ...
        conn.getName = (jid, some) => {
            ...
            if (!some) {
                let v = id === jidNormalizedUser(conn.user.id) ? conn.user : ((conn.contacts ? conn.contacts[id] : {}) || {});
    ...
    if (m.message) {
        ...
        m.quoted = m.msg?.contextInfo?.quotedMessage ? {
            key: {
                remoteJid: m.chat,
                fromMe: jidNormalizedUser(m.msg.contextInfo.participant) === jidNormalizedUser(conn.user.id),
