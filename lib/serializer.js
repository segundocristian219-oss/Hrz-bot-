...
export const smsg = async (conn, m) => {
    ...
    if (!conn.downloadM) {
        ...
        conn.getName = (jid, some) => {
            ...
            if (!some) {
                let v = id === sId(conn.user.id) ? conn.user : ((conn.contacts ? conn.contacts[id] : {}) || {});
       ...
       if (m.key) {
        ...
        m.author = sId(m.key.participant || m.key.remoteJid || m.participant || conn.user.id);
        m.sender = await getRealJid(conn, m.author, m);

        const settings = conn.settings || {};
        const botName = settings.botName || (global.name ? global.name() : 'Kirito');

        m.pushName = m.fromMe ? botName : (m.pushName || m.verifiedName || conn.getName(m.sender));
    ...
    if (m.message) {
        ...
        m.msg = (m.mtype === 'viewOnceMessageV2' || m.mtype === 'viewOnceMessage') ? m.message[m.mtype].message[getContentType(m.message[m.mtype].message)] : m.message[m.mtype];
        ...
        m.mentionedNames = await Promise.all(m.mentionedJid.map(async (jid) => {
            ...
            if (name.includes('@') || /^\d+$/.test(name)) {
                let userDb = await global.User.findOne({ id: sId(jid) });
        ...
        m.quoted = m.msg?.contextInfo?.quotedMessage ? {
            key: {
                remoteJid: m.chat,
                fromMe: sId(m.msg.contextInfo.participant) === sId(conn.user.id),
        ...
        if (m.quoted) {
            ...
            if (m.quoted.msg) {
                let rawQuotedText = '';
                if (m.quoted.mtype === 'interactiveResponseMessage') {
                    const qParams = JSON.parse(m.quoted.msg.nativeFlowResponseMessage?.paramsJson || '{}');
                    rawQuotedText = qParams.id || m.quoted.msg.body?.text || '';
                } else {
                    rawQuotedText = m.quoted.msg?.text || m.quoted.msg?.caption || m.quoted.msg?.contentText || m.quoted.message?.conversation || '';
                }
                m.quoted.text = typeof rawQuotedText === 'string' ? rawQuotedText.trim() : '';
                m.quoted.author = sId(m.msg.contextInfo.participant);
