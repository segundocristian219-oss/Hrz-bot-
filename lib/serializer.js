...
export const smsg = async (conn, m) => {
    ...
    if (!conn.downloadM) {
        ...
        conn.reply = async (jid, text = '', quoted, options = {}) => {
            let adReply = null;
            if (options.adReply) {
                const { data: imageBuffer } = await conn.getFile(options.adReply.image || global.img());
                adReply = {
                    title: options.adReply.title || global.nameBot,
                    body: options.adReply.body || 'Sistema Operativo',
                    mediaType: 2,
                    previewType: 'NONE',
                    thumbnail: imageBuffer,
                    sourceUrl: options.adReply.url || 'https://dix.lat/channel',
            ...
            return conn.sendMessage(jid, { 
                ...
                contextInfo: adReply ? { externalAdReply: adReply } : (options.contextInfo || {})
            }, { quoted: quoted || m, ...options });
        };
    }
