...
export const smsg = async (conn, m) => {
    ...
    const mentions = [...text.matchAll(/@([0-9]{5,16}|[a-zA-Z0-9._-]+)/g)].map(v => v[1]);
    
    return mentions.map(v => v.includes('@') ? v : v + '@s.whatsapp.net');
};
    ...
    const mentions = options.mentions ? options.mentions : conn.parseMention(text);
