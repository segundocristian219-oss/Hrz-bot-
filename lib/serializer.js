...
export const smsg = async (conn, m) => {
    ...
    if (!some) {
        let v = id === conn.user.id ? conn.user : (conn.contacts[id] || {})
        return v.name || v.subject || v.verifiedName || v.notify || id.split('@')[0]
    } else {
    ...
    if (m.message) {
        ...
        m.mentionedNames = m.mentionedJid.map(jid => conn.getName(jid));
        ...
        m.quoted = m.msg?.contextInfo?.quotedMessage ? {
            ...
            m.quoted.pushName = conn.getName(m.quoted.sender);
