...
export const smsg = async (conn, m) => {
    ...
    if (!some) {
        ...
        conn.reply = (jid, text = '', quoted, options) => {
            return conn.sendMessage(jid, { text: text || '', mentions: conn.parseMention(text || '') }, { quoted: quoted || m, ...options });
        };
    }
    ...
    if (m.message) {
        ...
        m.mentionedJid = await resolveMentions(conn, rawMentions, m);
                m.mentionedNames = m.mentionedJid.map(jid => {
            let name = conn.getName(jid);
            if (name.includes('@') || /^\d+$/.test(name)) {
                return global.db.data?.users[jid]?.name || name;
            }
            return name;
        });
        ...
        m.quoted = m.msg?.contextInfo?.quotedMessage ? {
            ...
            m.quoted.pushName = conn.getName(m.quoted.sender) || global.db.data?.users[m.quoted.sender]?.name || m.msg.contextInfo.pushName || m.quoted.sender.split('@')[0];
