    ...
    run: async (m, { conn, command, usedPrefix }) => {
        if (command === 'code') {
            ...
            if (!phoneNumber || phoneNumber.replace(/\D/g, '').length < 8) {
                return m.reply('> *⚠️ Número de teléfono inválido.*')
            }
            ...
            if (!result) return m.reply('> *❌ Error al generar el código.*')
            if (result.status === 'already_connected') return m.reply('> *✅ Sesión ya activa.*')
            if (result.status === 'code') {
                return conn.sendMessage(m.chat, {
                    text: `*───「 CÓDIGO 」───*\n\n\`\`\`${result.code}\`\`\`\n\n_Ingresa este código en WhatsApp._`
                }, { quoted: m }).catch(() => {})
            }
        }
...
export async function assistant_accessJadiBot(options) {
    ...
    if (findActiveConn(id)) return { status: 'already_connected' }
...
function setupSubBotEvents(sock, authFolder) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            console.log(chalk.bold.greenBright(`[SUB-BOT] ONLINE: ${id}`))
        ...
        if (connection === 'close') {
            ...
            removeFromPool(sock)
            console.log(chalk.bold.yellow(`[SUB-BOT] ${id}: ${DISCONNECT_MESSAGES[reason] || reason}`))
