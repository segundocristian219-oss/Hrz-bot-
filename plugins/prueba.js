...
const threadsCommand = {
    ...
    run: async (m, { conn, args, usedPrefix, command }) => {
        ...
        try {
            ...
            let mediaItems = [];
            let description = html.match(/<meta[^>]*property="og:description"[^>]*content="([^"]*)"/)?.[1] || 'Threads Content';
            ...
            if (mediaItems.length === 0) {
                ...
                for (const match of videoMatches) mediaItems.push({ type: 'video', url: cleanUrl(match[1]) });
            ...
            const caption = `\t\t*── 「 THREADS DOWNLOAD 」 ──*\n\n` +
                             `▢ *INFO:* ${description}\n` +
                             `▢ *CANTIDAD:* ${mediaItems.length}\n\n`;

            if (mediaItems.length > 1) {
                const albumMedias = mediaItems.map(item => ({
                    type: item.type,
                    data: { url: item.url }
                }));

                await sendAlbum(conn, m.chat, albumMedias, {
                    caption: caption,
                    quoted: m,
                    delay: 500
                });
                ...
                const res = await axios.get(item.url, { responseType: 'arraybuffer', headers });
                await conn.sendMessage(m.chat, {
                    [item.type]: Buffer.from(res.data, 'binary'),
                    caption: caption,
...
function extractMediaFromJson(obj, results = [], depth = 0) {
    ...
    if (Array.isArray(obj.carousel_media)) {
        for (const item of obj.carousel_media) extractMediaFromJson(item, results, depth + 1);
    }
    for (const val of Object.values(obj)) {
        if (val && typeof val === 'object') extractMediaFromJson(val, results, depth + 1);
    }
    return results;
}
...
async function sendAlbum(conn, jid, medias, options = {}) {
    ...
    for (let i = 0; i < medias.length; i++) {
        ...
        if (i < medias.length - 1) await new Promise(r => setTimeout(r, options.delay || 500));
    }
}
