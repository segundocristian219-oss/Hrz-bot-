...
function encryptZip(zipBuffer) {
    ...
    const mac = crypto.createHmac('sha256', macKey)
        .update(iv).update(encrypted).digest().slice(0, 10);
...
function buildZip(files) {
    ...
    for (const file of files) {
        ...
        cd.writeUInt32LE(size, 20); cd.writeUInt32LE(size, 24);
...
const stickerPackSearch = {
    ...
    run: async (m, { conn, text }) => {
        ...
        try {
            ...
            // 3. Descargar cover + stickers en paralelo
            ...
            // 6. Subir a WhatsApp usando waUploadToServer
