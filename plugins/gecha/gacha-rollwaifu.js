...
let handler = async (m, { conn, args, usedPrefix, command, isROwner }) => {
  ...
  async function buscarImagenDelirius(query) {
    ...
    console.log(`[wimage] Buscando con tag: "${tag}"`)
    ...
    for (const src of sourcesaleatory) {
      try {
        ...
        if (!res.ok) {
          console.log(`[wimage] ${src.name} HTTP ${res.status}`)
          continue
        }

        const type = res.headers.get("content-type") || ""
        if (!type.includes("json")) {
          console.log(`[wimage] ${src.name} Respuesta no JSON`)
          continue
        }
        ...
        console.log(`[wimage] ${src.name} → ${images.length} imagen(es) encontrada(s)`)

        if (images.length) {
          return Array.from(new Set(images))
        }
      } catch (err) {
        console.log(`[wimage] ${src.name} Error`, err.message)
      }
    }
  ...
  try {
    ...
    const group = global.db.data.groupGacha[m.chat] =
      global.db.data.groupGacha[m.chat] || {}
    ...
    const groupUser =
      group.users[m.sender] || (group.users[m.sender] = {})
    ...
    if (!validImages.length)
      return m.reply(
        `❖ No se encontraron imágenes para *${character.name}*.`
      )
    ...
    const fileName = `${character.name}${ext}`
