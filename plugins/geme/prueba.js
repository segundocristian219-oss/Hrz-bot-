...
const treasureGame = {
    ...
    async before(m) {
        ...
        if (txt === game.target) {
            game.board[y][x] = '💎';
            await m.react("💰");
            let finalMap = game.board.map(row => row.join(' ')).join('\n');
            await this.reply(m.chat, `🎊 ¡@${m.sender.split('@')[0]} ENCONTRASTE EL TESORO EN *${game.target}*!\n\n${finalMap}\n\nRecompensa: El honor de ser el mejor buscador.`, m, { mentions: [m.sender] });
            ...
            if (game.attempts >= 5) {
                game.board[parseInt(game.target[1]) - 1][game.target.charCodeAt(0) - 65] = '💎';
                let loseMap = game.board.map(row => row.join(' ')).join('\n');
                await this.reply(m.chat, `💀 *GAME OVER*\n\nSe agotaron las excavaciones. El tesoro estaba en *${game.target}*.\n\n${loseMap}`, m);
                delete global.treasureGames[m.chat];
                return true;
            }

            let currentMap = game.board.map(row => row.join(' ')).join('\n');
    ...
    run: async (m, { conn }) => {
        global.treasureGames = global.treasureGames || {};
        if (global.treasureGames[m.chat]) return conn.reply(m.chat, `⚠️ Ya hay una búsqueda en curso en este chat.`, m);
        ...
        let initialMap = board.map(row => row.join(' ')).join('\n');
